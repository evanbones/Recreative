...
Instead of providing a string (e.g., `"minecraft:diamond"`), you provide an object with an `item` field, and optionally an `after`, `before`, or `nbt` field.

#### Example: Inserting after an item with NBT
```json
{
  "action": "modify_tab",
